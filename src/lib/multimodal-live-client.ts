...
export class MultimodalLiveClient extends EventEmitter<MultimodalLiveClientEventTypes> {
  ...
  sendRealtimeInput(chunks: { mimeType: string; data: string }[]) {
    const message: RealtimeInputMessage = {
      realtimeInput: chunks,
    };
    this._sendDirect(message);
  }

  /**
   * send text or multimodal to the server and return a function to cancel it
   */
  send(text: string) {
  ...
  protected _sendDirect(data: object) {
    if (this.ws?.readyState === WebSocket.OPEN) {
      this.ws.send(JSON.stringify(data));
    } else {
      console.warn("Tried to send on a non-open socket!", data);
    }
  }
} 